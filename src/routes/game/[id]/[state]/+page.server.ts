...
export const load: PageServerLoad = async (event) => {
  ...
  if (!game.players.map((player: { email: any; }) => player.email).includes(session?.user?.email)) {
    let message = { game_id: params.id || '', email: session?.user?.email || '', deck_id: '', card_ids: '' };
...
export const actions = {
  ...
  GAME_SET: async(event) => {
    ...
    let session = await event.locals.auth();
    let message = { game_id: params.id || '', email: session?.user?.email || '', deck_id:  deckId?.toString() || '', card_ids: ''};
  ...
  RESPOND: async(event) => {
    ...
    let session = await event.locals.auth();
    let message = { game_id: params.id || '', email: session?.user?.email || '', deck_id: '', card_ids: cardIds?.toString() || '' };
